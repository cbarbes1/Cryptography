...
static const int powersOfTen[] = { 1, 10, 100, 1000, 10000, 100000, 1000000,
		10000000, 100000000 };
...
inline InfInt::InfInt(long long l) :
		pos(l >= 0) {
	//PROFINY_SCOPE
	bool subtractOne = false;
	if (l == LONG_LONG_MIN) {
...
inline InfInt::InfInt(unsigned int l) :
		pos(true) {
	//PROFINY_SCOPE
	do {
...
inline InfInt::InfInt(unsigned long l) :
		pos(true) {
	//PROFINY_SCOPE
	do {
		val.push_back((ELEM_TYPE) (l % BASE));
...
inline InfInt::InfInt(unsigned long long l) :
		pos(true) {
	//PROFINY_SCOPE
	do {
		val.push_back((ELEM_TYPE) (l % BASE));
...
inline InfInt::InfInt(const InfInt &l) :
		pos(l.pos), val(l.val) {
	//PROFINY_SCOPE
}
...
inline InfInt InfInt::operator*(const InfInt &rhs) const {
	...
	for (;; ++digit) {
		...
		for (size_t i = digit < rhs.val.size() ? 0 : digit - rhs.val.size() + 1;
			...
			PRODUCT_TYPE pval = result.val[digit]
					+ val[i] * (PRODUCT_TYPE) rhs.val[digit - i];
	...
	result.correct();
	result.pos =
			(result.val.size() == 1 && result.val[0] == 0) ?
					true : (pos == rhs.pos);
	return result;
}
...
inline void InfInt::correct(bool justCheckLeadingZeros, bool hasValidSign) {
	...
	if (!justCheckLeadingZeros) {
		...
		if (equalizeSigns()) {
			pos = ((val.size() == 1 && val[0] == 0) || !hasValidSign) ?
					true : pos;
		} else {
